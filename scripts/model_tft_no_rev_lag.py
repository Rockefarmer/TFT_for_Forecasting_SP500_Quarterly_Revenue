...
def _load_all(data_dir: str) -> pd.DataFrame:
    ...
    # REMOVED: revenue lag feature creation
...
def choose_known_future_cols(df: pd.DataFrame):
    """Calendar + lag1 asset/equity (prefer *_log if present)."""
...
def build_feature_lists(df: pd.DataFrame):
    ...
    # Base unknown features - REMOVED R&D FEATURES AND REVENUE LAGS
    base_unknown = [
        "revenue_log",
        # REMOVED: "revenue_log_lag1", "revenue_log_lag4",
...
def main():
    ...
    # UPDATED: Only require revenue_log, not the lag features
    REQ_LAGS = ["revenue_log"]  # Only require current revenue_log, not lags
