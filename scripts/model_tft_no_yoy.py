...
def build_feature_lists(df: pd.DataFrame):
    ...
    # Base unknown features - REMOVED R&D FEATURES
    base_unknown = [
        "revenue_log",
...
def _sanitize_encoder_features(df: pd.DataFrame, encoder_cols: list) -> tuple[pd.DataFrame, list]:
    ...
    for c in encoder_cols:
        ...
        # REMOVED: Special handling for YoY features
        # All features now use groupwise median, then 0.0 fallback
